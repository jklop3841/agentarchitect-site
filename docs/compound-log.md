...
## 2026-04-20：CSDN adapter 探测被网络层阻断

- 新增 `scripts/csdn_dry_run_probe.py`。
- CSDN 编辑器、首页、登录页在当前环境均无法稳定打开。
- 错误包括 `ERR_CONNECTION_CLOSED`、`ERR_TIMED_OUT`、`ERR_ABORTED`。
- 结论：暂不继续 CSDN 选择器适配，先切换到知乎/百家号，避免卡住主线。

## 2026-04-20：知乎 adapter 需要人工登录

- `scripts/zhihu_dry_run_probe.py` 可以打开知乎写作入口。
- 当前被重定向到登录页，包含手机号、短信验证码、易盾验证码。
- 结论：不能绕过，必须用户人工登录一次后再探测编辑器选择器。
